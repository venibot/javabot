    ...
    public void onGuildInviteCreate(GuildInviteCreateEvent inviteCreateEvent) {
        ...
        if (logChannel != null) {
            ...
            logEmbed.addField("Кто создал", String.format("%s(%s)",
                    inviteCreateEvent.getInvite().getInviter().getAsMention(), inviteCreateEvent.getInvite().getInviter().getAsTag()));
            logEmbed.addField("Максимальное количество использований",
                    inviteCreateEvent.getInvite().getMaxUses() != 0 ? String.valueOf(inviteCreateEvent.getInvite().getMaxUses()) : "Не ограничено");
