...
@DiscordCommand(name = "warns", description = "Посмотреть варны пользователя", aliases = {"варны", "преды"}, usage = "[Пользователь]",
...
public class WarnsCommand implements Command {
    ...
    public void doCommand(MessageReceivedEvent msg_event, String[] arguments) {
        try {
            Member member = arguments.length > 0 ? Converters.getMember(msg_event.getGuild(), arguments[0]) : msg_event.getMember();
            ...
            if (warns.size() == 0) {
                ...
                for (Warn warn : warns) {
                    infoEmbed.addField("ID " + warn.getWarnID() + " - " + msg_event.getJDA().getUserById(warn.getPunisherID()).getAsTag() + " - " + warn.getReason(),
                            "Предупреждение выдано " + DataFormatter.datetimeToString(DataFormatter.unixToDateTime(warn.getWarnTime())) + ". "
                                    + (warn.getEndTime() != 0 ? "Предупреждение истечёт " + DataFormatter.datetimeToString(DataFormatter.unixToDateTime(warn.getEndTime())) : "Предупреждение перманентное"));
                }
            }
