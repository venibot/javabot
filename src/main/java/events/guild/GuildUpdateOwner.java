    ...
    public void onGuildUpdateOwner(GuildUpdateOwnerEvent updateOwnerEvent) {
        ...
        if (logChannel != null) {
            ...
            logEmbed.addField("Старый владелец", String.format("%s(%s)",
                    updateOwnerEvent.getOldOwner().getAsMention(), updateOwnerEvent.getOldOwner().getUser().getAsTag()));
            logEmbed.addField("Новый владелец", String.format("%s(%s)",
                    updateOwnerEvent.getNewOwner().getAsMention(), updateOwnerEvent.getNewOwner().getUser().getAsTag()));
